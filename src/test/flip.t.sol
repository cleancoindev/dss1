...
contract Vat_ is Vat {
    ...
    function tao_balance(address usr) public view returns (uint) {
        return tao[usr];
    }
    bytes32 ilk;
...
contract FlipTest is DSTest {
    ...
    function test_tend() public {
        ...
        assertEq(vat.tao_balance(ali),   199 ether);
        // gal receives payment
        assertEq(vat.tao_balance(gal),     1 ether);

        Guy(bob).tend(id, 100 ether, 2 ether);
        // bid taken from bidder
        assertEq(vat.tao_balance(bob), 198 ether);
        // prev bidder refunded
        assertEq(vat.tao_balance(ali), 200 ether);
        // gal receives excess
        assertEq(vat.tao_balance(gal),   2 ether);
    ...
    function test_dent() public {
        ...
        assertEq(vat.tao_balance(ali),  150 ether);
        assertEq(vat.tao_balance(bob),  200 ether);
    }
    ...
    function test_yank_tend() public {
        ...
        assertEq(vat.tao_balance(ali),            200 ether);
        assertEq(vat.tao_balance(address(this)),    0 ether);
        // gems go to caller
