...
contract EndTest is DSTest {
    ...
    function tao(address urn) internal view returns (uint) {
        return vat.tao(urn) / RAY;
    }
    ...
    function init_collateral(bytes32 name) internal returns (Ilk memory) {
        ...
        // 1 coin = 6 tao and liquidation ratio is 200%
    ...
    function test_cage_collateralised() public {
        ...
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 tao
    ...
    function test_cage_undercollateralised() public {
        ...
        // bob's urn has 0 gem, 1 ink, 3 tab, 3 tao
    ...
    function test_cage_skip() public {
        ...
        gold.gemA.join(urn1, 10 ether);
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        // this urn has 0 gem, 10 ink, 15 tab, 15 tao
        ...
        gold.flip.tend(auction, 10 ether, rad(1 ether)); // bid 1 tao
        assertEq(tao(urn1), 14 ether);
        ...
        assertEq(tao(address(this)), 1 ether);       // bid refunded
        vat.move(address(this), urn1, rad(1 ether)); // return 1 tao to ali
        ...
        // balance the vow
        vow.heal(min(vat.tao(address(vow)), vat.sin(address(vow))));
        // global checks:
        assertEq(vat.debt(), rad(15 ether));
        assertEq(vat.vice(), rad(15 ether));
    ...
    function test_cage_undercollateralised_surplus() public {
        ...
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 tao
        // alive gives one tao to the vow, creating surplus
    ...
    function test_cage_net_undercollateralised_multiple_ilks() public {
        ...
        gold.pip.poke(bytes32(2 * WAD));
        // urn1 has 20 tao of ink and 15 tao of tab
        coal.pip.poke(bytes32(2 * WAD));
        // urn2 has 2 tao of ink and 5 tao of tab
        end.cage();
        end.cage("gold");
        end.cage("coal");
        end.skim("gold", urn1);  // over-collateralised
        ...
        // the gold is worth 15 tao and the coal is worth 2 tao
        // the total collateral pool is worth 17 tao
        // the total outstanding debt is 20 tao
        // each tao should get (15/2)/20 gold and (2/2)/20 coal
