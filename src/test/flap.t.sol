...
contract FlapTest is DSTest {
    ...
    function test_kick() public {
        assertEq(vat.tao(address(this)), 1000 ether);
        assertEq(vat.tao(address(flap)),    0 ether);
        flap.kick({ lot: 100 ether
                  , bid: 0
                  });
        assertEq(vat.tao(address(this)),  900 ether);
        assertEq(vat.tao(address(flap)),  100 ether);
    }
    function test_tend() public {
        ...
        assertEq(vat.tao(address(flap)),  0 ether);
        assertEq(vat.tao(bob), 100 ether);
        // income is burned
