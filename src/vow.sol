...
contract VatLike {
    function tao (address) external view returns (uint);
...
contract Vow is LibNote {
    ...
    function heal(uint rad) external note {
        require(rad <= vat.tao(address(this)), "Vow/insufficient-surplus");
    ...
    function flop() external note returns (uint id) {
        ...
        require(vat.tao(address(this)) == 0, "Vow/surplus-not-zero");
        Ash = add(Ash, sump);
        id = flopper.kick(address(this), dump, sump);
    }
    // Surplus auction
    function flap() external note returns (uint id) {
        require(vat.tao(address(this)) >= add(add(vat.sin(address(this)), bump), hump), "Vow/insufficient-surplus");
    ...
    function cage() external note auth {
        ...
        Sin = 0;
        Ash = 0;
        flapper.cage(vat.tao(address(flapper)));
        flopper.cage();
        vat.heal(min(vat.tao(address(this)), vat.sin(address(this))));
    }
}
