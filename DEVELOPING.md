# Multi Collateral Tao Developer Guide

*work in progress*

This is a more in depth description of the Tao core contracts. The
previous iteration of Tao was called Single Collateral Tao (SCD), or
...
Tao has three different numerical units: `wad`, `ray` and `rad`
...
The core CDP, Tao, and collateral state is kept in the `Vat`. This
contract has no external dependencies and maintains the central
"Accounting Invariants" of Tao.

Tao cannot exist without collateral:
...
CDP of user `u`, using `gem` from user `v` and creating `tao` for user
`w`.
...
equal quantity of Tao using `heal(u, v, rad)`: take `sin` from `u` and
`tao` from `v`.

Note that `heal` can also be used to *create* Tao, balanced by an equal
quantity of Sin.

Finally, the quantity `tao` can be transferred between users with `move`.
...
which increases the `tao` balance of the user `u` by increasing the
...
- `spot`: the maximum amount of Tao drawn per unit collateral
- `line`: the maximum total Tao drawn

And a global risk parameter:

- `Line`: the maximum total Tao drawn across all ilks
